...
public class ItemRepositoryCustomImpl implements ItemRepositoryCustom {  // 1-1. ItemRepositoryCustom 인터페이스 상속
     ...
     *		ex) searchDataType 값이 "1m"인 경우 dateTime의 시간을 한달 전으로 세팅 후 최근 한달 동안 등록된 상품만 조회하도록 조건값을 반환합니다.
	 */
	...
	public Page<Item> getAdminItemPage(ItemSearchDto itemSearchDto, Pageable pageable) {
		...
		/*
		 * - 첫번째 방법 -
		long totalCount = queryFactory.select(Wildcard.count).from(QItem.item)
											.where(regDtsAfter(itemSearchDto.getSearchDateType()),
												   searchSellStatusEq(itemSearchDto.getSearchSellStatus()),
												   searchByLike(itemSearchDto.getSearchBy(), itemSearchDto.getSearchQuery()))
											.orderBy(QItem.item.id.desc())
		...
		// 1-8. 게시글 전체 수 조회(위에 results로 size를 하면 해당 페이지에 나타나는 갯수만 가져오므로 queryDsl 5.0.0이상부터 이렇게 해줘야한다)
	...
	/*
	 * 1-9. 검색어가 null이 아니면 상품명에 해당 검색어가 포함되는 상품을 조회하는 조건을 반환합니다. 
	 */
	 ...
	 */
		...
		return new PageImpl<MainItemDto>(results, pageable, totalCount);
	}
