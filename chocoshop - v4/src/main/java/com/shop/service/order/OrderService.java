...
public class OrderService {
	...
	public Page<OrderHistDto> getOrderList(String email, Pageable pageable) {
		...
		for(Order order : orders) {
			// 2-7. 주문서(Order)를 바탕으로 구매이력 페이지에 전달할 주문목록(OrderHistDto) 객체를 만든다.
			OrderHistDto orderHistDto = new OrderHistDto(order);
			
			// 2-8. 주문아이템 리스트 객체를 만들어 주문서(Order)에서 주문목록을 가져와서 세팅 
			List<OrderItem> orderItems = order.getOrderItems();
			
			// 2-9. 주문서(Order)에서 가져온 주문아이템들을 향상된 for문으로 돌린다.
			for(OrderItem orderItem : orderItems) {
				// 2-10. 주문서에 등록된 아이템의 아이디와 대표이미지인지 알려주는 "Y"를 가지고 itemImgRepository에서 대표 이미지를 가져온다.
				ItemImg itemImg = itemImgRepository.findByItemIdAndRepImgYn(orderItem.getItem().getId(), "Y");
				
				// 2-11. 뷰 페이지로 넘겨주기 위해 주문아이템(orderItem)과 대표이미지 URL로 OrderItemDto 객체를 만든다.
				OrderItemDto orderItemDto = new OrderItemDto(orderItem, itemImg.getImgUrl());
				
				// 2-12. 2-11에서 만든 OrderItemDto 객체를 주문 내역 조회 페이지로 넘겨줄때 사용할 orderHistDto에 세팅.
				orderHistDto.addOrderItemDto(orderItemDto);
			}
			// 2-13. 반환할 orderHistDto 리스트에 세팅.
	...
	public boolean validateOrder(Long orderId, String email) {
		...
		// 3-5. 현재 로그인한 사용자와 주문을 한 사용자가 동일한지 비교한다. 같으면 true, 같지 않으면 false
