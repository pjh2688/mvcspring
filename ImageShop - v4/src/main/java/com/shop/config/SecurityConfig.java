...
//				.access("request.method == 'GET' ? permitAll : hasRole('ROLE_USER') or hasRole('ROLE_MEMBER') or hasRole('ROLE_ADMIN')") // 1-12. ROLE_USER, ROLE_MEMBER, ROLE_ADMIN 허용
//				.antMatchers("/api/manage/**")  // 1-13. /api/manage/** 형식으로 주소가 들어오면
//				.access("request.method == 'GET' ? permitAll : hasRole('ROLE_MEMBER') or hasRole('ROLE_ADMIN')")  // 1-14.ROLE_MEMBER, ROLE_ADMIN 허용
				.antMatchers("/api/admin/**", "/api/codegroups/**", "/api/codedetails/**")  // 1-15. /api/admin/**, /api/codegroups/**, /api/codedetails/** 형식으로 주소가 들어오면
				.access("request.method == 'GET' ? permitAll : hasRole('ROLE_ADMIN')")  // 1-16. GET방식 이외에 요청은 ROLE_ADMIN만 허용
				.antMatchers("/api/users/**", "/api/boards/**")  // 1-23. /api/users/**, /api/boards/** 요청은
				.access("request.method == 'GET' ? permitAll : hasRole('ROLE_MEMBER') or hasRole('ROLE_ADMIN')")  // 1-24. ROLE_MEMBER, ROLE_ADMIN만 허용
				.antMatchers("/api/notices/**")  // 1-25. /api/notices로 오는 모든 url은
				.access("request.method == 'GET' ? permitAll : hasRole('ROLE_ADMIN')") // 1-26. GET 방식만 허용하고 나머지는 ROLE_ADMIN 권한만 가능.
				.antMatchers("/api/items/**")  // 1-27. /api/items로 오는 모든 url은
				.access("request.method == 'GET' ? permitAll : hasRole('ROLE_ADMIN')") // 1-28. GET 방식만 허용하고 나머지는 ROLE_ADMIN 권한만 가능.
				.antMatchers("/api/coins/**")  // 1-29. /api/coins로 오는 모든 url은
				.access("hasRole('ROLE_USER') or hasRole('ROLE_MEMBER') or hasRole('ROLE_ADMIN')") // 1-30. ROLE_USER, ROLE_MEMBER, ROLE_ADMIN 권한만 가능.
				.antMatchers("/api/useritems/**")  // 1-31. /api/useritems/** 로 오는 모든 url은
				.access("hasRole('ROLE_USER') or hasRole('ROLE_MEMBER') or hasRole('ROLE_ADMIN')") // 1-32. ROLE_USER, ROLE_MEMBER, ROLE_ADMIN 권한만 가능
				.antMatchers("/api/pds/**")  // 1-33. /api/pds/**로 오는 모든 url은
				.access("request.method == 'GET' ? permitAll : hasRole('ROLE_ADMIN')") // 1-34. GET 방식만 허용하고 나머지는 ROLE_ADMIN 권한만 가능.
