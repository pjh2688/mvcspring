...
public class MemberService {
	...
	public int setupAdmin(Member member) {
		...
		if(mapper.countAll() == 0) {
			...
			int result = mapper.save(member);
			
			Member findMember = mapper.findByUserId(member.getUserId()).get();
	...
	public List<CodeLabelValue> getCodeGroupList(String groupCode) {
		...
		try {
			result = mapper.getCodeLabelValueList(groupCode);
	...
	public int register(Member member) {
		Optional<Member> memberOp = mapper.findByUserId(member.getUserId());
	...
	public Member findByUserNo(Long userNo) {
		...
		Optional<Member> memberOp = mapper.findByUserNo(userNo);
	...
	public int delete(Long userNo) {
		...
		if(memberOp.isPresent()) {
			...
			int result = mapper.delete(userNo);
