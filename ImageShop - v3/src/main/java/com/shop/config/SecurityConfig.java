...
public class SecurityConfig {
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		return http
				...
				.antMatchers("/api/admin/**", "/api/codegroups/**", "/api/codedetails/**")  // 1-15. /api/admin/**, /api/codegroups/** 형식으로 주소가 들어오면
				.access("request.method == 'GET' ? permitAll : hasRole('ROLE_ADMIN')")  // 1-16. ROLE_ADMIN만 허용
				.antMatchers("/api/users/**", "/api/boards/**")  // 1-23. boards 추가
				.access("request.method == 'GET' ? permitAll : hasRole('ROLE_MEMBER') or hasRole('ROLE_ADMIN')")
				.antMatchers("/api/notices/**")  // 1-24. /api/notices로 오는 모든 url은
				.access("request.method == 'GET' ? permitAll : hasRole('ROLE_ADMIN')") // 1-25. GET 방식만 허용하고 나머지는 ROLE_ADMIN 권한만 가능.
				.antMatchers("/api/items/**")  // 1-24. /api/notices로 오는 모든 url은
				.access("request.method == 'GET' ? permitAll : hasRole('ROLE_ADMIN')") // 1-25. GET 방식만 허용하고 나머지는 ROLE_ADMIN 권한만 가능.
				.antMatchers("/api/coins/**")  // 1-24. /api/notices로 오는 모든 url은
				.access("hasRole('ROLE_USER') or hasRole('ROLE_MEMBER') or hasRole('ROLE_ADMIN')") // 1-25. GET 방식만 허용하고 나머지는 ROLE_ADMIN 권한만 가능.
				.antMatchers("/api/useritems/**")  // 1-24. /api/notices로 오는 모든 url은
				.access("hasRole('ROLE_USER') or hasRole('ROLE_MEMBER') or hasRole('ROLE_ADMIN')") // 1-25. GET 방식만 허용하고 나머지는 ROLE_ADMIN 권한만 가능.
