...
public class ItemImgService {
	...
	public void saveItemImg(ItemImg itemImg, MultipartFile itemImgFile) throws Exception {
		...
		if(!StringUtils.isEmpty(oriImgName)) {
			...
			// 1-7. imgUrl : 업로드 결과 로컬에 저장된 상품이미지 파일을 불러오는 요청 경로, application.properties에 등록한 itemImgLocation에 uploadPath 경로에다가 /item 폴더 경로를 추가했으므로 /item을 붙여준다.
	...
	public void updateItemImg(Long itemImgId, MultipartFile itemImgFile) throws Exception {
		...
		if(!itemImgFile.isEmpty()) {
			...
			// 2-4. 2-3에서 조회한 데이터에 기존 이미지가 있으면 파일을 삭제
			if(!StringUtils.isEmpty(savedItemImg.getImgName())) {
				fileService.deleteFile(itemImgLocation + "/" + savedItemImg.getImgName()); 
			}
			
			// 2-5. 기존에 이미지가 없다면 전달 받은 MultipartFile itemImgFile의 원본 파일명을 가지고온다.
			String oriImgName = itemImgFile.getOriginalFilename();
